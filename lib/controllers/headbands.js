...
module.exports = Router()
  ...
  .put('/api/v1/headbands/:id', async (req, res) => {
    try {
      const headband = await Headband.updateItem(req.params.id);
      res.send(headband);
    } catch(err) {
      res.status(500).send(err);
    }
  ...
  .delete('/api/v1/headbands/:id', async (req, res) => {
    try {
      const headband = await Headband.deleteItem(req.params.id);
      res.send(headband);
    } catch(err) {
      res.status(500).send(err);
    }
