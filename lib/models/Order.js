...
class Order {
  ...
  static async putOrder(quantity_of_items) {
    const { rows } = await pool.query(
      'UPDATE orders SET (quantity_of_item) VALUES ($1) RETURNING  *',
      [quantity_of_items]
    );
    return new Order(rows[0]);
  }

  static async deleteOrder(quantity_of_items) {
    const { rows } = await pool.query(
      'DELETE orders SET (quantity_of_item) VALUES ($1) RETURNING  *',
      [quantity_of_items]
    );
    return new Order(rows[0]);
  }
